...
Examples:
    ...
    # Run Pipeline 1 with soft-run (count queries without calling OpenAI)
    python -m fly_stocker_v2.cli get-allele-refs ./gene_lists --soft-run
"""
    )
...
5. Outputs Excel file with stocks and references
    ...
    refs_parser.add_argument(
        "--test-log",
        action="store_true",
        help="Log all GPT queries to Logs/GPT-Queries/ directory"
    )
    refs_parser.add_argument(
