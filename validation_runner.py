    ...
    *,
    ...
    if stock_tasks_override is None:
        ...
        for _, row in stocks_with_refs.iterrows():
            ...
            relevant_alleles = parse_semicolon_list(row.get("relevant_component_symbols", ""))
            stock_is_custom[stock_num] = row.get("is_custom_stock", False)
            ...
            for allele in relevant_alleles:
                for pmid in component_to_pmids.get(allele, set()):
                    pmid_clean = clean_id(pmid)
                    if pmid_clean and pmid_clean in keyword_pmids:
                        all_pairs.append((allele, pmid_clean))
        ...
        for stock_num, tasks in stock_tasks_override.items():
            ...
            stock_is_custom[stock_clean] = row.get("is_custom_stock", False)
            gene_terms: List[str] = []
            seen_terms: Set[str] = set()
            for sym in parse_semicolon_list(row.get("relevant_gene_symbols", "")):
                sym_clean = str(sym).strip()
